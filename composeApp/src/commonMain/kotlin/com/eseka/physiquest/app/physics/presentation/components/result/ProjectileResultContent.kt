...
@Composable
fun ProjectileResultContent(result: PhysicsResult.ProjectileMotion) {
    var isSimulating by remember { mutableStateOf(false) }
    var showSimulation by remember { mutableStateOf(false) }
    Column {
        Text(
            text = buildString {
                append("Max Height: ${result.result.maxHeight.formatDecimal(2)} m\n")
                append("Range: ${result.result.range.formatDecimal(2)} m\n")
                append("Flight Time: ${result.result.flightTime.formatDecimal(2)} s\n")
                append("Time to Max Height: ${result.result.timeToMaxHeight.formatDecimal(2)} s\n")
                append("Velocity at Impact: ${result.result.velocityAtImpact.formatDecimal(2)} m/s")
            },
